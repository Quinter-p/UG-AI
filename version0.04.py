...
# /clear_memory          清空长期记忆
# /history               查看最近对话历史
# /history_status        查看历史文件状态
# /clear_history         清空短期对话历史
# /state                 查看当前角色/场景状态
# /scene                 查看当前场景状态
...
ENABLE_PERSISTENT_HISTORY = CONFIG["history"]["enable_persistent_history"]
HISTORY_FILE = CONFIG["history"]["history_file"]
MAX_SAVED_TURNS = CONFIG["history"]["max_saved_turns"]
LOAD_RECENT_TURNS = CONFIG["history"]["load_recent_turns"]
...
def append_persistent_history(user_text, assistant_text, source="chat"):
    if not ENABLE_PERSISTENT_HISTORY:
        return
...
def load_persistent_history_text():
    if not ENABLE_PERSISTENT_HISTORY:
        return ""

    return build_history_text(
        history_file=HISTORY_FILE,
        load_recent_turns=LOAD_RECENT_TURNS,
...
def main():
    ...
    history = load_persistent_history_text()
    debug_tool_router = DEBUG_TOOL_ROUTER_DEFAULT

    print("本地 AI 助手已启动。")
    print(f"当前模型：{MODEL_NAME}")
    print(f"RAG：{'开启' if ENABLE_AUTO_RAG else '关闭'}")
    print(f"短期历史持久化：{'开启' if ENABLE_PERSISTENT_HISTORY else '关闭'}")
    print(f"工具识别调试：{'开启' if debug_tool_router else '关闭'}")
    print("默认：流式输出。")
    print("输入 exit 退出。")
    print("输入 /history 查看最近保存的对话历史。")
    print("输入 /history_status 查看历史文件状态。")
    print("输入 /clear_history 清空短期对话历史。")
    print("输入 /tool_debug on 开启自然语言工具识别调试。")
    print("现在也支持部分自然语言工具调用，比如：读一下 version0.04.py、看看 knowledge 文件夹、记住 xxx、我们在哪。\n")
    ...
    while True:
        ...
        if user_text == "/history":
            text = format_recent_history(
                history_file=HISTORY_FILE,
                recent_turns=LOAD_RECENT_TURNS
            )
            print("\n" + text + "\n")
            write_log("HISTORY", text)
            continue

        if user_text == "/history_status":
            text = history_status(HISTORY_FILE)
            print("\n" + text + "\n")
            write_log("HISTORY_STATUS", text)
            continue

        if user_text == "/clear_history":
        ...
        if user_text in ["/rag_build", "/rag_update", "/reg_build"]:
            intent = {"action": "rag_update"}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/rag_rebuild":
            intent = {"action": "rag_rebuild"}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text.startswith("/rag_search"):
            query = user_text.replace("/rag_search", "", 1).strip()
            intent = {"action": "rag_search", "query": query}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/rag_status":
            intent = {"action": "rag_status"}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue
        ...
        if user_text == "/scene":
            intent = {"action": "scene_view"}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text.startswith("/set_scene"):
            scene_desc = user_text.replace("/set_scene", "", 1).strip()
            intent = {"action": "scene_set", "scene": scene_desc}
            handled, memory, state, _ = handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text.startswith("/scene_note"):
            note = user_text.replace("/scene_note", "", 1).strip()
            intent = {"action": "scene_note", "note": note}
            handled, memory, state, _ = handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/clear_scene":
            intent = {"action": "scene_clear"}
            handled, memory, state, _ = handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/memory":
            intent = {"action": "memory_show"}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/clear_memory":
            memory = clear_memory_func()
            reply = "长期记忆已清空。"
            print(f"AI：{reply}\n")
            write_log("AI", reply)
            continue
        ...
        if user_text.startswith("/remember"):
            ...
            intent = {"action": "memory_add", "fact": fact}
            handled, memory, state, history_reply = handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            if history_reply:
                append_persistent_history(user_text, history_reply, source="tool")
                history = load_persistent_history_text()
            continue

        if user_text.startswith("/files"):
            path = user_text.replace("/files", "", 1).strip() or "."
            intent = {"action": "file_list", "path": path}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text.startswith("/read"):
            path = user_text.replace("/read", "", 1).strip()
            intent = {"action": "file_read", "path": path}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text.startswith("/search"):
            query = user_text.replace("/search", "", 1).strip()
            intent = {"action": "knowledge_search", "query": query}
            handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            continue

        if user_text == "/logs":
            intent = {"action": "logs_view"}
            handled, memory, state, history_reply = handle_natural_tool_intent(intent, user_text, llm_client, memory, state)
            if history_reply:
                append_persistent_history(user_text, history_reply, source="tool")
                history = load_persistent_history_text()
        ...
        try:
            ...
            append_persistent_history(user_text, assistant_text, source="chat")
            history = load_persistent_history_text()
